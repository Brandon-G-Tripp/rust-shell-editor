use crate::terminal::{Terminal, set_cursor_position};
...
pub fn handle_motion(motion: Motion) {
    match motion {
...
fn move_left<W: Write>(terminal: &mut Terminal<W>) -> io::Result<()> {
    let (mut x, y) = terminal.get_cursor_position();
    if x > 0 {
        x -= 1;
        set_cursor_position(&mut terminal.writer, x, y)?;
    } 
    Ok(())
} 

fn move_right<W: Write>(terminal: &mut Terminal<W>) -> io::Result<()> {
    let (mut x, y) = terminal.get_cursor_position();
    x += 1;
    set_cursor_position(&mut terminal.writer, x, y)?;
    Ok(())
...
fn move_up<W: Write>(terminal: &mut Terminal<W>) -> io::Result<()> {
...
fn move_down<W: Write>(terminal: &mut Terminal<W>) -> io::Result<()> {
    let (x, mut y) = terminal.get_cursor_position();
    y += 1;
    set_cursor_position(&mut terminal.writer, x, y)?;
    Ok(())
